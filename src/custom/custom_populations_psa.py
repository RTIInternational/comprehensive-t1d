...
def parse_json(scenario, diabetes_type):
    if diabetes_type == 't1d':
        ...
        for data in data_dict_list:
            ...
            value_dict['proportion less than secondary'] = education_demographics[
                'proportion_less_than_secondary_education']
...
def generate_t1d_population(rng_list, scenario):
    rng = rng_list[0]
    psa_rng = rng_list[1]
    people_list = []
    value_dict_list = parse_json(scenario, 't1d')
    factor_changes, factor_contribution, basic_intervention_cost = get_risk_factor_changes(psa_rng, scenario, 't1d')
    if scenario['is_intervention']:
...
def generate_prediabetes_population(rng_list, scenario):
    rng = rng_list[0]
    psa_rng = rng_list[1]
    ...
    non_complier_columns = ['lifestyle_non_compliant', 'glycemic_non_compliant', 'cholesterol_non_compliant', 'bp_non_compliant']
    return people_df, non_complier_columns


def generate_prediabetes_screening_population(rng_list, scenario):
    rng = rng_list[0]
    psa_rng = rng_list[1]
    ...
    for _ in range(value_dict['pop size']):
        ...
        person_dict['normoglycemia'] = "[0]" if person_dict['prediabetes'] == "[1]" or person_dict[
            'diabetes'] == "[1]" else "[1]"
        ...
        # note: basic intervention only apply to pre-diabetic individuals; that's why this block of code has
        # been moved down here compared to the pre-diabetes scenario
    ...
    non_complier_columns = ['lifestyle_non_compliant', 'glycemic_non_compliant', 'cholesterol_non_compliant', 'bp_non_compliant']
    intervention_columns = ['baseline lifestyle intervention', 'baseline generic intervention', 'baseline glycemic intervention',
                            'baseline cholesterol intervention', 'baseline bp intervention']
...
def generate_custom_population(seed, scenario_tuple, equation_set):
    ...
    if equation_set == 't1d':
        ...
        population_df, _ = generate_t1d_population([rng, psa_rng], scenario_tuple[0])
        for scenario in scenario_tuple:
            ...
            if 'non' in scenario['scenario_name']:
                ...
                intervention_df, intervention_columns = generate_t1d_population([rng, psa_rng], scenario)
        ...
        rng = np.random.default_rng(seed)
        psa_rng = np.random.default_rng(seed)
        population_df, _ = generate_prediabetes_population([rng, psa_rng], scenario_tuple[0])
        for scenario in scenario_tuple:
            rng = np.random.default_rng(seed)
            psa_rng = np.random.default_rng(seed)
            if 'non' in scenario['scenario_name']:
                ...
                intervention_df, intervention_columns = generate_prediabetes_population([rng, psa_rng], scenario)
                mod_population_df = population_df.drop(columns=intervention_columns)
                intervention_df = intervention_df[intervention_columns]
                mod_population_df = mod_population_df.join(intervention_df)
                mod_population_df = mod_population_df[population_df.columns]
        ...
        rng = np.random.default_rng(seed)
        psa_rng = np.random.default_rng(seed)
        population_df, _ = generate_prediabetes_screening_population([rng, psa_rng], scenario_tuple[0])
        for scenario in scenario_tuple:
            rng = np.random.default_rng(seed)
            psa_rng = np.random.default_rng(seed)
            if 'non' in scenario['scenario_name']:
                ...
                intervention_df, intervention_columns = generate_prediabetes_screening_population([rng, psa_rng], scenario)
                mod_population_df = population_df.drop(columns=intervention_columns)
                intervention_df = intervention_df[intervention_columns]
                mod_population_df = mod_population_df.join(intervention_df)
                mod_population_df = mod_population_df[population_df.columns]
