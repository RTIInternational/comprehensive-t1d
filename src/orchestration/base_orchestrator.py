...
def write_semaphore(uuid, status):
    status_table = DATABASE['api_status']
    data = dict(uuid=uuid, status=status)
    status_table.upsert(data, ['uuid'])
...
class BaseOrchestrator():
    ...
    def get_population_path(self, popfile):
        ...
        return os.path.join(self.population_path, f'{popfile}.parquet')
