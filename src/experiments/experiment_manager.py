...
class ExperimentManager:
    ...
    def init_scenario(base_scenario, suffix, run_id):
        ...
        uuid = scenario.population_dataset_uuid
        ...
        if scenario.diabetes_type == 't1d':
            if isinstance(uuid, str) and uuid != 'none':
                scenario.demographics = {}
                scenario.population_composition = {}
                scenario.SEARCH_demographics = {}
                scenario.EXCHANGE_demographics = {}
                scenario.SEARCH_risk_factors = {}
                scenario.EXCHANGE_risk_factors = {}
                scenario.SEARCH_clinical_history = {}
                scenario.EXCHANGE_clinical_history = {}
            else:
                scenario.population_composition = base_scenario['population_composition']
                scenario.SEARCH_demographics = base_scenario['SEARCH_demographics']
                scenario.EXCHANGE_demographics = base_scenario['EXCHANGE_demographics']
                scenario.SEARCH_risk_factors = base_scenario['SEARCH_risk_factors']
                scenario.EXCHANGE_risk_factors = base_scenario['EXCHANGE_risk_factors']
                scenario.SEARCH_clinical_history = base_scenario['SEARCH_clinical_history']
                scenario.EXCHANGE_clinical_history = base_scenario['EXCHANGE_clinical_history']
        elif scenario.diabetes_type == 'pre':
            if isinstance(uuid, str) and uuid != 'none':
                scenario.demographics = {}
            ...
            scenario.cvd_multipliers = base_scenario['cvd_multipliers']
            scenario.cvd_costs = base_scenario['disutilities_prediabetes'] + base_scenario['costs_prediabetes']
            ...
            scenario.cvd_multipliers = base_scenario['cvd_multipliers']
            scenario.cvd_costs = base_scenario['disutilities_prediabetes'] + base_scenario['costs_prediabetes']
            scenario.who_gets_screened = base_scenario['who_gets']
            scenario.non_screening_diagnosis = base_scenario['probability_of_nonscreening_diagnosis']
            scenario.screening_characteristics = base_scenario['screening_characteristics']
            scenario.annual_prob_prediabetes = base_scenario['annual_probability_of_developing_prediabetes']
        else:
            if isinstance(uuid, str) and uuid != 'none':
                scenario.demographics = {}
                scenario.risk_factors = {}
                        ...
                        for risk, value in risk_reductions.items():
                            if value != 0:
                                ...
                                if value < 1:
                                    ...
                                    log_value = -18.42
                                new_value = normal.model(rng, log_value, abs(log_value) / z_dict[int(psa_risk)], 4)
    ...
    def generate_scenarios(cls, base_scenario):
        ...
        for run_id in range(1, base_scenario['iterations'] + 1):
            ...
            for run_type in ['__non-intervention', '__intervention']:
                ...
                surgery_dict = {'intervention_name': 'surgery control intervention'}
                ...
                intervention_type_mapping = {'generic': 'generic_control_intervention', 'glycemic': 'glycemic_control_intervention',
                                             ...
                                             'surgery': 'surgery_control_intervention'}
                all_interventions = {}
                if run_type == '__intervention':
                ...
                # advanced interventions are also applied to non-intervention runs
                else:
                    for intervention_type in intervention_type_dicts.keys():
                        if f'{intervention_type}_control_setup_type' in base_scenario.keys():
                            setup_type = f'{intervention_type}_control_setup_type'
                            if base_scenario[setup_type] == 'advanced':
                                intervention_dict = intervention_type_dicts[intervention_type]
                                for key, value in base_scenario.items():
                                    if intervention_type in key:
                                        intervention_dict[key] = value
                ...
                if base_scenario['simulation_type'] == 'PSA':
                    ...
                    # advanced interventions are also applied to non-intervention runs
                        ...
                        advanced_interventions = cls.psa_advanced_interventions(psa_rng, interventions,
                                                                                psa_factor,
                                                                                psa_cost)
                        # note: the second dictionary takes precedence
                        intervention_set['interventions'][0] = advanced_interventions
