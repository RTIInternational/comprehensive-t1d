...
class ComprehensiveModel:
    ...
    def set_equations(self, equation_set):
        ...
        # added to self.seed to guarantee that each equation gets a different seed
        shift_values = self.rng.integers(0, 1000, size=1000)
        for stage in set_module.STAGES:
            ...
            for eq in stage:
                eq_seed_index = self.rng.integers(0,1000)
                ...
                try:
                    ...
                    if self.scenario['scenario_type'] == 'PSA' and self.scenario['diabetes_type'] == 'screen' and eq == 'screening':
                        rng = np.random.default_rng(self.seed + shift_values[eq_seed_index])
                        module.modify_coefficients({'who_gets_screened': self.scenario['who_gets_screened'],
                                                    'screening_characteristics':
                                                        self.scenario['screening_characteristics']}, rng)
                    elif self.scenario['scenario_type'] == 'PSA' and self.scenario['diabetes_type'] == 'screen' and eq == 'non-screening-diagnosis':
                        rng = np.random.default_rng(self.seed + shift_values[eq_seed_index])
                        module.modify_coefficients({'non_screening_diagnosis': self.scenario['non_screening_diagnosis']}, rng)
                    elif self.scenario['scenario_type'] == 'PSA':
                        if 'mortality' in eq:
                            mortality_rng = np.random.default_rng(self.scenario['seed'] + shift_values[eq_seed_index])
                            module.modify_coefficients(self.scenario['psa_mortality_variability'], mortality_rng)
                        else:
                            complication_rng = np.random.default_rng(self.scenario['seed'] + shift_values[eq_seed_index])
                ...
                if should_process:
                    ...
                    if eq in risk_reduction_dict.keys():
                        ...
                        rng = np.random.default_rng(self.seed + shift_values[eq_seed_index])
                        equation = EquationFactory(rng, equation_set, module, self.uuid, self.custom_values, reductions, is_intervention)
                        ...
                        rng = np.random.default_rng(self.seed + shift_values[eq_seed_index])
                        equation = EquationFactory(rng, equation_set, module, self.uuid, None, hba1c_intervention, is_intervention)
                        ...
                        rng = np.random.default_rng(self.seed + shift_values[eq_seed_index])
                        equation = EquationFactory(rng, equation_set, module, self.uuid, None, cholesterol_intervention, is_intervention)
                        ...
                        rng = np.random.default_rng(self.seed + shift_values[eq_seed_index])
                        equation = EquationFactory(rng, equation_set, module, self.uuid, None, sbp_intervention, is_intervention)
                        ...
                        if is_intervention:
                            ...
                            rng = np.random.default_rng(self.seed + shift_values[eq_seed_index])
                            equation = EquationFactory(rng, equation_set, module, self.uuid, None, smoking_intervention, is_intervention)
                        else:
                            rng = np.random.default_rng(self.seed + shift_values[eq_seed_index])
                            equation = EquationFactory(rng, equation_set, module, self.uuid, None, None, is_intervention)
                    elif eq == 'complication_prediabetes':
                        rng = np.random.default_rng(self.seed + shift_values[eq_seed_index])
                        equation = EquationFactory(rng, equation_set, module, self.uuid, self.custom_values, self.interventions, is_intervention)
                        ...
                        rng = np.random.default_rng(self.seed + shift_values[eq_seed_index])
                        equation = EquationFactory(rng, equation_set, module, self.uuid, self.custom_values, lifestyle_intervention, is_intervention)
                    elif eq == 'bariatric_surgery' and 'surgery_control_intervention' in self.interventions.keys():
                        surgery_intervention = self.interventions['surgery_control_intervention']
                        rng = np.random.default_rng(self.seed + shift_values[eq_seed_index])
                        equation = EquationFactory(rng, equation_set, module, self.uuid, self.custom_values, surgery_intervention, is_intervention)
                    elif 'economics' in eq:
                        rng = np.random.default_rng(self.seed + shift_values[eq_seed_index])
                        equation = EquationFactory(rng, equation_set, module, self.uuid, self.economics, self.interventions, is_intervention)
                    # only applies to pre-diabetes; make sure to not override complications with basic interventions
                        ...
                        rng = np.random.default_rng(self.seed + shift_values[eq_seed_index])
                        equation = EquationFactory(rng, equation_set, module, self.uuid, self.custom_values, None, is_intervention)
    ...
    def set_population(self, population_name):
        ...
        population_path_parquet = os.path.join(INPUT_DIR, self.uuid, f'{population_name}.parquet')
        # population = pd.read_csv(population_path)
        # population = pd.read_pickle(population_path_pickle)
        population = pd.read_parquet(population_path_parquet)
    ...
    def write_population(self):
        ...
        # original setup was to save file to a simulation directory and then copy them over to the analysis directory
        # we can skip the simulation directory and just save to the analysis directory
        OUTPUT_DIR = os.path.join(BASE_DIR, 'analysis', 'output', self.uuid, 'data')
        # OUTPUT_DIR = os.path.join(BASE_DIR, 'simulation', 'output', self.uuid)

        if not os.path.exists(OUTPUT_DIR):
            os.makedirs(OUTPUT_DIR)

        csv_output_path = os.path.join(OUTPUT_DIR, f'{self.scenario_name}.csv')
        output_path = os.path.join(OUTPUT_DIR, f'{self.scenario_name}.pkl')
        output_path_parquet = os.path.join(OUTPUT_DIR, f'{self.scenario_name}.parquet')
        ...
        if os.path.isfile(output_path_parquet):
            os.remove(output_path_parquet)
        pl.DataFrame(sim_df, infer_schema_length=1_000).write_parquet(output_path_parquet)